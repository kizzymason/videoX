    ...
    const [rows, countRows] = await Promise.all([
      db
        .select({
          ...
          reportCount: t.comments.reportCount,
          createdAt: t.comments.createdAt,
          videoId: t.comments.videoId,
          videoTitle: t.videos.title,
          authorName: t.users.displayName,
          authorUsername: t.users.username,
        })
        .from(t.comments)
        .leftJoin(t.videos, eq(t.videos.id, t.comments.videoId))
        .leftJoin(t.users, eq(t.users.id, t.comments.userId))
        .where(where)
        .orderBy(desc(t.comments.reportCount), desc(t.comments.createdAt))
        .limit(q.pageSize)
        .offset((q.page - 1) * q.pageSize),
...
adminRouter.post(
  ...
  validate({ params: idParam, body: z.object({ status: z.enum(['visible', 'hidden', 'deleted']) }) }),
  asyncHandler(async (req, res) => {
    const { id } = params<{ id: string }>(req);
    ...
    await db.transaction(async (tx) => {
      ...
      if (wasVisible !== nowVisible) {
        await tx
          .update(t.videos)
          .set({
            commentCount: nowVisible
              ? sql`${t.videos.commentCount} + 1`
              : sql`greatest(0, ${t.videos.commentCount} - 1)`,
          })
...
adminRouter.get(
  '/categories',
  asyncHandler(async (_req, res) => {
    const rows = await db.select().from(t.categories).orderBy(t.categories.sortOrder, t.categories.name);
    ok(res, rows);
  }),
);
...
adminRouter.patch(
  '/categories/:id',
  validate({ params: idParam, body: categorySchema.partial() }),
  asyncHandler(async (req, res) => {
    const { id } = params<{ id: string }>(req);
    const [row] = await db
      .update(t.categories)
      .set({ ...body<Record<string, unknown>>(req), updatedAt: new Date() } as never)
...
adminRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [used] = await db
      ...
      .from(t.videos)
      .where(eq(t.videos.categoryId, id));
...
adminRouter.get(
  ...
  asyncHandler(async (req, res) => {
    const { page, pageSize } = query<{ page: number; pageSize: number }>(req);
    const [rows, countRows] = await Promise.all([
      db.select().from(t.tags).orderBy(desc(t.tags.videoCount)).limit(pageSize).offset((page - 1) * pageSize),
      db.select({ total: sql<number>`count(*)::int` }).from(t.tags),
    ]);
...
adminRouter.get(
  '/banners',
  asyncHandler(async (_req, res) => {
    const rows = await db.select().from(t.banners).orderBy(t.banners.sortOrder);
    ok(res, rows);
  }),
);
...
adminRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [row] = await db
      ...
      .values({
        ...input,
        startsAt: input.startsAt ? new Date(input.startsAt as string) : null,
        endsAt: input.endsAt ? new Date(input.endsAt as string) : null,
      } as never)
      .returning();
...
adminRouter.patch(
  '/banners/:id',
  validate({ params: idParam, body: bannerSchema.partial() }),
  asyncHandler(async (req, res) => {
    const { id } = params<{ id: string }>(req);
    const input = body<Record<string, unknown>>(req);
...
adminRouter.patch(
  '/plans/:id',
  validate({ params: idParam, body: planSchema.partial() }),
  asyncHandler(async (req, res) => {
    const { id } = params<{ id: string }>(req);
    const [row] = await db
      .update(t.plans)
...
adminRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [used] = await db
      .select({ c: sql<number>`count(*)::int` })
      .from(t.redeemCodes)
      .where(eq(t.redeemCodes.planId, id));
...
adminRouter.get(
  '/redeem-codes',
  validate({ query: redeemCodeQuerySchema }),
  asyncHandler(async (req, res) => {
...
adminRouter.post(
  ...
  validate({ body: generateCodesSchema }),
  asyncHandler(async (req, res) => {
    const input = body<{
      planId: string;
      count: number;
...
adminRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [row] = await db
      ...
      .where(and(eq(t.redeemCodes.id, id), eq(t.redeemCodes.status, 'unused')))
      .returning();
    ...
    const [rows, countRows] = await Promise.all([
      db
        .select({
          ...
          planId: t.orders.planId,
          amountCents: t.orders.amountCents,
          source: t.orders.source,
          status: t.orders.status,
          redeemCodeId: t.orders.redeemCodeId,
          note: t.orders.note,
          channelPayload: t.orders.channelPayload,
          createdAt: t.orders.createdAt,
          updatedAt: t.orders.updatedAt,
          username: t.users.username,
          planName: t.plans.name,
        })
        .from(t.orders)
...
adminRouter.get(
  '/storage',
  asyncHandler(async (_req, res) => {
    ok(res, await listStorageProfiles());
  }),
);

adminRouter.post(
  '/storage',
  validate({ body: storageProfileSchema }),
  asyncHandler(async (req, res) => {
...
adminRouter.patch(
  '/storage/:id',
  validate({ params: idParam, body: storageProfileSchema.partial() }),
  asyncHandler(async (req, res) => {
    const { id } = params<{ id: string }>(req);
...
adminRouter.post(
  '/storage/:id/test',
  validate({ params: idParam, body: storageProfileSchema.partial().optional() }),
  asyncHandler(async (req, res) => {
    const { id } = params<{ id: string }>(req);
    const override = body<{ driver?: 'local' | 's3'; config?: Record<string, unknown> } | undefined>(req);
    const result = await testStorageProfile(
      id,
      override?.driver && override.config ? { driver: override.driver, config: override.config } : undefined,
    );
...
adminRouter.get(
  '/settings/algo',
  asyncHandler(async (_req, res) => {
    ok(res, await getAlgoWeights());
  }),
);

adminRouter.put(
  '/settings/algo',
  validate({ body: algoWeightsSchema }),
  asyncHandler(async (req, res) => {
...
adminRouter.patch(
  '/ai/profiles/:id',
  validate({ params: idParam, body: aiProfileSchema.partial() }),
  asyncHandler(async (req, res) => {
    const { id } = params<{ id: string }>(req);
    const input = body<Record<string, unknown>>(req);
    // 掩码值代表「未修改」，不要用它覆盖真实密钥。
    if (input.apiKey === '••••••••' || input.apiKey === '') delete input.apiKey;

    const [row] = await db
      .update(t.aiProfiles)
      .set({ ...input, updatedAt: new Date() } as never)
      .where(eq(t.aiProfiles.id, id))
      .returning();
...
adminRouter.post(
  '/ai/profiles/:id/run',
  validate({ params: idParam, body: z.object({ videoIds: z.array(z.string()).max(500).optional() }).optional() }),
  asyncHandler(async (req, res) => {
    const { id } = params<{ id: string }>(req);
    ...
    const [run] = await db
      .insert(t.aiScoringRuns)
      .values({ profileId: id, status: 'running' })
      .returning();
...
adminRouter.get(
  ...
  asyncHandler(async (_req, res) => {
    const rows = await db
      .select({
        id: t.aiScoringRuns.id,
        profileId: t.aiScoringRuns.profileId,
        profileName: t.aiProfiles.name,
        status: t.aiScoringRuns.status,
        totalVideos: t.aiScoringRuns.totalVideos,
        scoredVideos: t.aiScoringRuns.scoredVideos,
        errorMessage: t.aiScoringRuns.errorMessage,
        startedAt: t.aiScoringRuns.startedAt,
        finishedAt: t.aiScoringRuns.finishedAt,
      })
      .from(t.aiScoringRuns)
      .leftJoin(t.aiProfiles, eq(t.aiProfiles.id, t.aiScoringRuns.profileId))
      .orderBy(desc(t.aiScoringRuns.startedAt))
      .limit(20);
...
adminRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [rows, countRows] = await Promise.all([
      db
        .select({
          ...
          aiScore: t.videos.aiScore,
          aiReason: t.videos.aiReason,
          aiScoredAt: t.videos.aiScoredAt,
          viewCount: t.videos.viewCount,
        })
        .from(t.videos)
        .where(sql`${t.videos.aiScore} is not null`)
        .orderBy(desc(t.videos.aiScore))
        .limit(pageSize)
        .offset((page - 1) * pageSize),
      db
        .select({ total: sql<number>`count(*)::int` })
    ...
    ok(
      res,
      paginated(
        rows.map((r) => ({ ...r, aiScoredAt: r.aiScoredAt?.toISOString() ?? null })),
        Number(countRows[0]?.total ?? 0),
        page,
        pageSize,
      ),
